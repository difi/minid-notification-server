...
public class ApprovalEndpoint {
    ...
    public ResponseEntity<String> approve(@RequestHeader HttpHeaders headers, @RequestBody AuthorizationApprovalEntity approvalEntity, @AuthenticationPrincipal OAuth2AuthenticatedPrincipal principal) {
        AuthorizationNotificationEntity notification = notificationCache.getApprovalNotificationForLoginAttempt(approvalEntity.getRequestId());
        ...
        if (false) {
            ...
            return authenticationService.sendApproval(notification.getPersonIdentifier());
        }
    ...
    @PostMapping("/reject")
    @PreAuthorize("hasAuthority('SCOPE_minid:app.register')") //TODO: Inntil videre. Skal byttes ut SNART
    public ResponseEntity<String> reject(@RequestHeader HttpHeaders headers, @RequestBody AuthorizationApprovalEntity approvalEntity, @AuthenticationPrincipal OAuth2AuthenticatedPrincipal principal) {
        AuthorizationNotificationEntity notification = notificationCache.getApprovalNotificationForLoginAttempt(approvalEntity.getRequestId());
        if (notification == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Request not found");
        }
        ...
        if (false) {
            ...
            return authenticationService.sendRejection(notification.getPersonIdentifier());
        }
        return ResponseEntity.status(HttpStatus.NO_CONTENT).build();
    }

    private ValidateEntity getValidateEntity(AuthorizationNotificationEntity notificationEntity, String token) {
        return ValidateEntity.builder()
                .person_identifier(notificationEntity.getPersonIdentifier())
                .login_attempt_id(notificationEntity.getKey())
                .build();
